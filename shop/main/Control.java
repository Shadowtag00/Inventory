...
class Control {
  ...
  private UIFactory.UIForm[] _menus;
  private int _state;

  private UIFactory.UIForm _getVideoForm;
  ...
  Control(Inventory inventory, UI ui) {
    ...
    _menus = new UIFactory.UIForm[NUMSTATES];
    _state = START;
    addSTART(START);
    addEXIT(EXIT);
  ...
  private void addSTART(int stateNum) {
    ...
    m.add("Print top ten all time rentals in order",
      new UIMenuAction() {
        public void run() {
          // TODO
        	//_inventory.iterator().next().numRentals();
        	//for (Record i:_inventory.iterator()) {
    ...
    _menus[stateNum] = m.toUIForm("Bob's Video");
  }
  private void addEXIT(int stateNum) {
    ...
    _menus[stateNum] = m.toUIForm("Are you sure you want to exit?");
  }
}
