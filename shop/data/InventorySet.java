...
final class InventorySet implements Inventory {
  ...
  public Record get(Video v) {
    // TODO
	  for (Video i : _data.keySet()) {
		  //if (i.title().equals(v.title())&&i.director().equals(v.director())&&i.year()==v.year()) {
		  if(i.hashCode()==v.hashCode()) {
			 // _data.put(v, _data.get(i));
			  //_data.remove(i);
			  return _data.get(i);
		  }
	  }
  ...
  public Iterator<Record> iterator(Comparator<Record> comparator) {
    // TODO
	  List<Record> nw = new ArrayList<Record>(_data.values());
      Collections.sort(nw, comparator);
      return nw.iterator();
  }
  ...
  Record checkOut(Video video) {
    // TODO
	  Record rec = this.get(video);
	  if(rec==null) {
	    	throw new IllegalArgumentException("Video has no record");

	  }else if(rec.numOut()==rec.numOwned()) {
	    	throw new IllegalArgumentException("Max checked out");
  ...
  Record checkIn(Video video) {
    // TODO
	  ...
	  }else if(rec.numOut()==0) {
	    	throw new IllegalArgumentException("Max checked in");

	  }else {
		  //rec.numOut=rec.numOut-1;
  ...
  Map<Video,Record> clear() {
    // TODO
	  Map<Video, Record> x = new HashMap<Video,Record>();
	  
	  for (Entry<Video, Record> i :_data.entrySet()) {
